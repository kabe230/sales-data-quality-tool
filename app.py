...
SAMPLE_PATTERNS = {
    "all_valid": {
        "label": "① すべて正常",
        "description": (
            "100件すべてが正常です。指摘0件の状態と、6種類のステータスを含む"
            "各種集計を確認できます。"
        ),
    ...
    "auto_cleanup": {
        "label": "② 自動整形を体験",
        "description": (
            "100件中、正常90件・警告5件・エラー5件です。全8種類・1,042件の"
            "自動修正に加え、追加列の保持と空行除外も確認できます。"
        ),
    ...
    "quality_issues": {
        "label": "③ エラー・警告を確認",
        "description": (
            "100件中、正常45件・警告5件・エラー50件です。必須、形式、重複、"
            "金額、日付など全14種類の検証結果をまとめて確認できます。"
        ),
