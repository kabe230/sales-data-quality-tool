...
公開デモでは、ファイルを用意しなくても次の3パターンを1クリックで確認できます。

| パターン | 確認できる内容 | 実行結果 |
|---|---|---|
| すべて正常 | 100件の正常データと6種類のステータスを含む各種集計 | 正常100・警告0・エラー0 |
| 自動整形を体験 | 全8種類の自動修正、追加列の保持、空行除外 | 正常90・警告5・エラー5・自動修正1,042件 |
| エラー・警告を確認 | 必須・形式・重複・金額・日付など全14種類の検証 | 正常45・警告5・エラー50 |

選択したサンプルはUTF-8 BOM付きCSVとしてダウンロードできます。ExcelやVS Codeで
値を編集し、画面の「自分のファイルを使う」から再アップロードできます。
...
画面の「サンプルデータで試す」から3パターンを選ぶと、すぐに動作を確認できます。
サンプルCSVは `python scripts/generate_sample_data.py` で同じ内容を再生成できます。
